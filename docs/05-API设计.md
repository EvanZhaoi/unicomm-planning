...
- 测试阶段后端通过临时人员缓存校验用户，不接入真实人员 API
- 生产阶段后端会通过 `PersonnelProvider` 接入公司人员系统获取员工信息
- 如果人员数据返回有效员工并且状态正常，则创建会话 Token
- 如果人员数据返回失败/用户不存在/用户停用，则拒绝访问
- 后续可把人员信息写入 `user_snapshot` 作为缓存和审计，`user_snapshot` 不能作为人员主数据
...
**注意事项:**
- 用户身份必须以后端人员适配器返回结果为准；测试阶段为 mock 人员源，生产阶段为真实公司人员接口
...
**说明:** 前端相关人选择器使用该接口按姓名、用户名、工号或邮箱模糊搜索员工成员。当前测试阶段数据源为后端临时员工缓存，不接入真实人员 API；后续通过统一 `PersonnelProvider.searchMembers()` 替换为公司人员接口。

**未来真实接口接入要求:**
- 成员搜索与桌面端认证使用同一个人员适配器，避免搜索结果和登录身份来源不一致
- 后端负责把真实人员接口字段映射为前端需要的 `username / employeeNo / displayName / departmentName / email`
- 搜索接口需要限制 `limit`、关键词长度和调用频率
- 生产环境不允许从前端直接传入任意人员信息写入相关人，只能选择后端人员接口返回的合法员工
